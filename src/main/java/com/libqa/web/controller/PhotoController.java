...
import com.libqa.web.service.photo.PhotoService;
...
public class PhotoController {
    ...
    @Autowired
    private PhotoService spaceService;
    ...
    @RequestMapping("/photo/main")
    public ModelAndView spaceMain(HttpServletRequest request) {
        log.debug("## /main");
        ModelAndView mav = new ModelAndView("/photo/main");

        // 전체 space 목록 조회 (10개)
        boolean isDeleted = false;
        boolean morePage = false;   // 더보기 여부
        PhotoMainList spacePages = spaceService.findPageBySort(isDeleted, LibqaConstant.PAGE_START_INDEX,
        ...
        if (user == null || user.isGuest()) {
             ...
             * 내 즐겨찾기 공간 정보 조회
             */
            List<Photo> myFavoriteSpaceList = spaceService.findUserFavoriteSpace(user.getUserId(), false);
            List<PhotoMain> favoriteSpaces;
         ...
         */
        PhotoWikiList spaceWikiPages = spaceService.findWikiPageBySort(isDeleted, LibqaConstant.PAGE_START_INDEX,
     ...
     * @return
     */
    @RequestMapping(value = "/photo/more", method = RequestMethod.GET)
    @ResponseBody
    public ResponseData<PhotoMainList> findMoreSpaceList(@RequestParam String sortType,
                                                         @RequestParam Integer startNum,
                                                         @RequestParam Integer pageSize) {
        PhotoMainList spaceMainList = spaceService.findPageBySort(false, startNum, pageSize, sortType);
     ...
     * @return
     */
    @RequestMapping(value = "/photo/morewiki", method = RequestMethod.GET)
    @ResponseBody
    public ResponseData<PhotoWikiList> findMoreWikis(@RequestParam String sortType,
                                                     @RequestParam Integer startNum,
                                                     @RequestParam Integer pageSize) {
        PhotoWikiList spaceWikiList = spaceService.findWikiPageBySort(Boolean.FALSE, startNum, pageSize, sortType);
     ...
     * @return
     */
    @RequestMapping(value = "/photo/render", method = RequestMethod.GET)
    @ResponseBody
    public ResponseData<PhotoMainList> renderSpace(@RequestParam String sortType) {
        log.info("## sortType = {}", sortType);

        PhotoMainList spaceMainList = spaceService.findPageBySort(false, 0, LibqaConstant.SPACE_PAGE_SIZE, sortType);
        return ResponseData.createSuccessResult(spaceMainList);

    }

    @PreAuthorize("hasAnyAuthority('USER', 'ADMIN')")
    @RequestMapping("/photo/form")
    public ModelAndView form(Model model) {
        ...
        ModelAndView mav = new ModelAndView("/photo/form");
    ...
    @ResponseBody
    public ResponseData<Photo> saveSpace(@ModelAttribute Photo space, @ModelAttribute Keyword keyword) throws IllegalAccessException {
        User user = loggedUserManager.getUser();
        ...
        Photo result = spaceService.saveWithKeyword(space, keyword, ActivityType.CREATE_SPACE);
     ...
     */
    @PreAuthorize("hasAnyAuthority('USER', 'ADMIN')")
    @RequestMapping(value = "/photo/edit/{spaceId}", method = RequestMethod.GET)
    public ModelAndView editSpace(@PathVariable Integer spaceId) throws IllegalAccessException {
        Photo space = spaceService.findOne(spaceId);
        User user = loggedUserManager.getUser();
        ...
        ModelAndView mav = new ModelAndView("/photo/edit");
        mav.addObject("user", user);
     ...
     */
    public void checkAuthorize(Photo space, User user) throws IllegalAccessException {
        if (user == null) {
            throw new IllegalAccessException("로그인 정보가 필요합니다.");
        }
    ...
    @ResponseBody
    public ResponseData<Photo> updateSpace(@ModelAttribute Photo space, @ModelAttribute Keyword keyword) throws IllegalAccessException {
        User user = loggedUserManager.getUser();
        Photo spaceEntity = spaceService.findOne(space.getSpaceId());
        ...
        Photo result = spaceService.saveWithKeyword(spaceEntity, keyword, ActivityType.UPDATE_SPACE);
        log.debug("#result : [{}]", result);
        return ResponseData.createSuccessResult(result);
    }
    ...
    @ResponseBody
    public ResponseData<Photo> deleteSpace(@RequestParam Integer spaceId) throws IllegalAccessException {
        User user = loggedUserManager.getUser();
        Photo result = null;

        boolean canDelete = false;      // 삭제 가능 여부
        boolean existWiki = false;      // 하위 위키 존재 여부
        Photo spaceEntity = spaceService.findOne(spaceId);
     ...
     * @param spaceEntity
     */
    public void bindSpaceEntity(Photo space, User user, Photo spaceEntity) {
     ...
     * @return
     */
    @RequestMapping(value = "/photo/{spaceId}", method = RequestMethod.GET)
    public ModelAndView spaceDetail(@PathVariable Integer spaceId) {
        ...
        for (Wiki wiki : updatedWikis) {
            ...
            PhotoWiki spaceWiki = new PhotoWiki();
     ...
     * @return
     */
    public boolean canDeleted(Photo space, User user) {
     ...
     */
    @PreAuthorize("hasAnyAuthority('USER', 'ADMIN')")
    @RequestMapping(value = "/photo/addFavorite", method = RequestMethod.GET)
    @ResponseBody
     ...
     */
    @PreAuthorize("hasAnyAuthority('USER', 'ADMIN')")
    @RequestMapping(value = "/photo/cancelFavorite", method = RequestMethod.GET)
    @ResponseBody
    ...
    @RequestMapping(value = "/photo/tree", method = RequestMethod.POST, produces = "text/html; charset=utf8")
    @ResponseBody
    ...
    @RequestMapping(value = "/photo/spaces", method = RequestMethod.GET)
    public ModelAndView spaces(HttpRequest request) {
        ModelAndView mav = new ModelAndView("/photo/spaces");
        boolean isDeleted = false;
        List<Photo> spaceList = spaceService.findAllByCondition(isDeleted);

        List<PhotoMain> spaceMains;
        int totalCount = 0;
    ...
    @RequestMapping(value = "/photo/allUser", method = RequestMethod.GET)
    @ResponseBody
